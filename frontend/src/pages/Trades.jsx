...
export default function Trades() {
  ...
  const handleSave = async () => {
    try {
      ...
      if (editing) {
        await api.put(`/api/trades/${editing.id}`, payload);
      } else {
        await api.post("/api/trades/add-trade", payload);
      }
  ...
  try {
    await api.delete(`/api/trades/${showDeletePopup.id}`, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem("token")}`,
      },
    });
          ...
          <div className="modal-overlay">
            <div className="modal-box">
              ...
              <div className="flex flex-col gap-4">
                ...
                <select
  className="cyber-input rounded-2xl bg-slate-900 text-white"
  ...
  <option value="WAZIRX" className="bg-slate-900 text-white">WAZIRX</option>
                </select>
